...
const HTML_OUTPUT_DIR = path.join(__dirname, '../../exports/html_cr_gmatprep');
const FAILED_URLS_LOG_PATH = path.join(__dirname, '../../exports/failedUrls_cr_gmatprep.log');
...
const CHECKPOINT_FILE = path.join(__dirname, '../../exports/extraction_checkpoint_cr_gmatprep.json');
...
async function readExcelSheet() {
    ...
    const worksheet = workbook.getWorksheet('CR - GMATPREP');
    if (!worksheet) {
        throw new Error('Worksheet "CR - GMATPREP" not found');
    }
...
async function saveHtmlFile(result: ExtractedContent): Promise<void> {
    ...
    const fileName = `cr_gmatprep_${result.questionNumber}.html`;
...
<head>
    <meta charset="UTF-8">
    <title>CR GMATPREP Question ${result.questionNumber}</title>
    <style>
        body { 
...
<body>
    ...
    console.log(`Saved HTML for CR GMATPREP question ${result.questionNumber} to ${filePath}`);
}

async function main() {
    try {
        console.log('Starting HTML extraction with Puppeteer for CR GMATPREP Questions...');
        ...
        while (currentRow <= totalRows) {
            ...
            if (!questionNumber || questionNumber === 'CRITICAL REASONING - GMATPREP' || questionNumber === 'Q#') {
                currentRow++;
                continue;
            }
            
            console.log(`Row ${currentRow}: CR GMATPREP Question Number = ${questionNumber}`);
            
            // Skip if this question has already been processed
            if (processedQuestionNumbers.has(questionNumber)) {
                console.log(`Skipping CR GMATPREP question ${questionNumber} - already processed`);
                currentRow++;
                continue;
            }
            ...
            if (url && typeof url === 'string') {
                console.log(`Processing CR GMATPREP question ${questionNumber} from URL: ${url}`);
                ...
                try {
                    ...
                    processedCount++;
                    console.log(`Successfully extracted CR GMATPREP question ${questionNumber} (${processedCount}/${TEST_MODE ? TEST_LIMIT : 'all'})`);
                } catch (error) {
                    console.error(`Error extracting CR GMATPREP question ${questionNumber}:`, error);
                ...
                console.log(`No valid URL found for CR GMATPREP question ${questionNumber} at row ${currentRow}`);
            }
        ...
        console.log('\nExtraction Summary:');
        console.log(`Processed ${processedQuestionNumbers.size} CR GMATPREP questions`);
