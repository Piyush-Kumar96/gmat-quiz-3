...
const main = async () => {
  try {
    ...
    // Find Reading Comprehension questions with no options
    ...
    for (const question of questions) {
      ...
      try {
        ...
        // Determine which result to use for updating the question
        ...
        // Update the question in the database if we have valid options
        if (bestOptions.filter(o => o !== '').length >= 3) {
          await QuestionBagV2.findByIdAndUpdate(
            question._id,
            { 
              options: bestOptions,
              questionText: bestQuestionText 
            }
          );
          console.log(`Updated question ${question._id} with extracted options and cleaned question text`);
        } else {
          console.log(`Failed to extract valid options for question ${question._id}`);
        }
    ...
    const resultsPath = path.join(exportPath, 'rc_options_extraction_results.json');
    const exportData = {
