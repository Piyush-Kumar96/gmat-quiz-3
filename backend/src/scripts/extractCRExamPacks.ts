...
const HTML_OUTPUT_DIR = path.join(__dirname, '../../exports/html_cr_exampacks');
const FAILED_URLS_LOG_PATH = path.join(__dirname, '../../exports/failedUrls_cr_exampacks.log');
...
const CHECKPOINT_FILE = path.join(__dirname, '../../exports/extraction_checkpoint_cr_exampacks.json');
...
async function readExcelSheet() {
    ...
    const worksheet = workbook.getWorksheet('CR - Exam Packs');
    if (!worksheet) {
        throw new Error('Worksheet "CR - Exam Packs" not found');
    }
...
async function saveHtmlFile(result: ExtractedContent): Promise<void> {
    ...
    const fileName = `cr_exampacks_${result.questionNumber}.html`;
...
<head>
    <meta charset="UTF-8">
    <title>CR Exam Packs Question ${result.questionNumber}</title>
    <style>
        body { 
...
<body>
    ...
    console.log(`Saved HTML for CR Exam Packs question ${result.questionNumber} to ${filePath}`);
}

async function main() {
    try {
        console.log('Starting HTML extraction with Puppeteer for CR Exam Packs Questions...');
        ...
        while (currentRow <= totalRows) {
            ...
            if (!questionNumber || questionNumber === 'CRITICAL REASONING - EXAM PACKS' || questionNumber === 'Q#') {
                currentRow++;
                continue;
            }
            
            console.log(`Row ${currentRow}: CR Exam Packs Question Number = ${questionNumber}`);
            
            // Skip if this question has already been processed
            if (processedQuestionNumbers.has(questionNumber)) {
                console.log(`Skipping CR Exam Packs question ${questionNumber} - already processed`);
                currentRow++;
                continue;
            }
            ...
            if (url && typeof url === 'string') {
                console.log(`Processing CR Exam Packs question ${questionNumber} from URL: ${url}`);
                ...
                try {
                    ...
                    processedCount++;
                    console.log(`Successfully extracted CR Exam Packs question ${questionNumber} (${processedCount}/${TEST_MODE ? TEST_LIMIT : 'all'})`);
                } catch (error) {
                    console.error(`Error extracting CR Exam Packs question ${questionNumber}:`, error);
                ...
                console.log(`No valid URL found for CR Exam Packs question ${questionNumber} at row ${currentRow}`);
            }
        ...
        console.log('\nExtraction Summary:');
        console.log(`Processed ${processedQuestionNumbers.size} CR Exam Packs questions`);
