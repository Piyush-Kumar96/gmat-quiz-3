...
// Constants - MODIFIED FOR OG QUESTIONS
const EXCEL_FILE_PATH = path.join(__dirname, '../../materials/GMAT Official Questions Bank OG GMAT Prep.xlsx');
const HTML_OUTPUT_DIR = path.join(__dirname, '../../exports/html_rc_ogquestions');
const FAILED_URLS_LOG_PATH = path.join(__dirname, '../../exports/failedUrls_rc_ogquestions.log');
...
const CHECKPOINT_FILE = path.join(__dirname, '../../exports/extraction_checkpoint_rc_ogquestions.json');
...
// Column indices for Excel file - Same as other RC sheets
const COL_INDEX = {
    QUESTION_NUM: 2,
    SOURCE: 3,
    RC_NUMBER: 4,
...
async function readExcelSheet() {
    ...
    // Modified to use "RC - OG Questions" sheet
    const worksheet = workbook.getWorksheet('RC - OG Questions');
    if (!worksheet) {
        throw new Error('Worksheet "RC - OG Questions" not found');
    }
...
async function main() {
    try {
        console.log('Starting HTML extraction with Puppeteer for RC OG Questions...');
