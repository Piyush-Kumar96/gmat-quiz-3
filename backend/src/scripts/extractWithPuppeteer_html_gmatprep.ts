...
const HTML_OUTPUT_DIR = path.join(__dirname, '../../exports/html_specific');
...
const CHECKPOINT_FILE = path.join(__dirname, '../../exports/extraction_checkpoint.json');
...
async function extractContentFromPage(url: string, questionNumber: string, retries = 0): Promise<ExtractedContent> {
    ...
    try {
        ...
        // Extract only the relevant HTML content
        const questionHtml = await page.evaluate(() => {
            ...
            // 1. Extract the question text from .item.text
            const textDiv = document.querySelector('.item.text');
            if (textDiv) {
                const questionTextDiv = document.createElement('div');
                questionTextDiv.className = 'question-text';
                questionTextDiv.innerHTML = textDiv.innerHTML;
            ...
            return extractedContent.outerHTML;
        });
        ...
        return {
            ...
            difficultyLevel: '', // Will be filled in later
            topic: ''         // Will be filled in later
        };
...
async function main() {
    try {
        console.log('Starting HTML extraction with Puppeteer...');
        ...
        console.log('\nExtraction Summary:');
        let actualQuestionsCount = currentProcessedQuestions.length + allFailedUrls.length;
        if (reachedEndOfData) {
