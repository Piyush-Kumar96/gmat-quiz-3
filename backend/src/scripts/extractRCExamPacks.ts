...
const HTML_OUTPUT_DIR = path.join(__dirname, '../../exports/html_rc_exampacks');
const FAILED_URLS_LOG_PATH = path.join(__dirname, '../../exports/failedUrls_rc_exampacks.log');
...
const CHECKPOINT_FILE = path.join(__dirname, '../../exports/extraction_checkpoint_rc_exampacks.json');
...
async function readExcelSheet() {
    ...
    const worksheet = workbook.getWorksheet('RC - Exam Packs');
    if (!worksheet) {
        throw new Error('Worksheet "RC - Exam Packs" not found');
    }
...
async function processBatch(worksheet: ExcelJS.Worksheet, startRow: number, batchSize: number, processedQuestions: string[] = []) {
    ...
    for (let rowIndex = startRow; rowIndex <= endRow; rowIndex++) {
        ...
        // Debug the question number cell to see what we're getting
        const questionNumCell = row.getCell(COL_INDEX.QUESTION_NUM);
        console.log('Question Number Cell:', JSON.stringify(questionNumCell.value));
        
        // Extract question number safely
        let questionNumber = '';
        if (questionNumCell.value) {
            if (typeof questionNumCell.value === 'object') {
                // If it's an object, try to extract text or a property
                questionNumber = questionNumCell.value.toString();
            } else {
        ...
        promises.push(
            limitConcurrency(async () => {
                try {
                    ...
                    const rcNumber = row.getCell(COL_INDEX.RC_NUMBER).value?.toString() || '';
...
async function main() {
    try {
        console.log('Starting HTML extraction with Puppeteer for RC Exam Pack questions...');
