...
const HTML_OUTPUT_DIR = path.join(__dirname, '../../exports/html_specific_exampacks');
const FAILED_URLS_LOG_PATH = path.join(__dirname, '../../exports/failedUrls_exampacks.log');
...
const CHECKPOINT_FILE = path.join(__dirname, '../../exports/extraction_checkpoint_exampacks.json');
...
async function readExcelSheet() {
    ...
    const worksheet = workbook.getWorksheet('Quant - Exam Packs');
    if (!worksheet) {
        throw new Error('Worksheet "Quant - Exam Packs" not found');
    }
...
async function main() {
    try {
        console.log('Starting HTML extraction with Puppeteer for Exam Pack questions...');
